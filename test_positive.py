...
class TestPositive:
    def test_step1(self, make_folders, clear_folders, make_files):
        ...
        result1 = ssh_checkout("0.0.0.0", "user2", "1111",
                               ...
                               "Everything is Ok")
        result2 = ssh_checkout("0.0.0.0", "user2", "1111", "ls {}".format(data["folder_out"]),
    ...
    def test_step2(self, clear_folders, make_files):
        ...
        for item in make_files:
            res.append(ssh_checkout("0.0.0.0", "user2", "1111", "ls {}".format(data["folder_ext"]), item))
    ...
    def test_step3(self):
        ...
        """
        assert ssh_checkout("0.0.0.0", "user2", "1111", "cd {}; 7z t arx2.{}".format(data["folder_out"], data["type"]),
    ...
    def test_step4(self):
        ...
        assert ssh_checkout("0.0.0.0", "user2", "1111",
                            "cd {}; 7z u ../out/arx2.{}".format(data["folder_in"], data["type"]),
    ...
    def test_step5(self, clear_folders, make_files):
        ...
        for item in make_files:
            res.append(
                ssh_checkout("0.0.0.0", "user2", "1111", "cd {}; 7z l arx2.{}".format(data["folder_out"], data["type"]),
                             item))
    ...
    def test_step6(self, clear_folders, make_files, make_subfolder):
        ...
        res.append(ssh_checkout("0.0.0.0", "user2", "1111",
                                ...
                                "Everything is Ok"))
        res.append(ssh_checkout("0.0.0.0", "user2", "1111",
                                "cd {}; 7z x arx.{} -o{} -y".format(data["folder_out"], data["type"],
                                                                    data["folder_ext2"]), "Everything is Ok"))

        for item in make_files:
            res.append(ssh_checkout("0.0.0.0", "user2", "1111", "ls {}".format(data["folder_ext2"]), item))

        res.append(ssh_checkout("0.0.0.0", "user2", "1111", "ls {}".format(data["folder_ext2"]), make_subfolder[0]))
        res.append(ssh_checkout("0.0.0.0", "user2", "1111", "ls {}/{}".format(data["folder_ext2"], make_subfolder[0]),
                                make_subfolder[1]))
    ...
    def test_step8(self, clear_folders, make_files):
        ...
        res = []
        for item in make_files:
            res.append(ssh_checkout("0.0.0.0", "user2", "1111", "cd {}; 7z h {}".format(data["folder_in"], item),
                                    "Everything is Ok"))
            hash = ssh_get("0.0.0.0", "user2", "1111", "cd {}; crc32 {}".format(data["folder_in"], item)).upper()
            res.append(ssh_checkout("0.0.0.0", "user2", "1111", "cd {}; 7z h {}".format(data["folder_in"], item), hash))
